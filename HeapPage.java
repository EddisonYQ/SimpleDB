...
public class HeapPage implements Page {
	 ...
	 */
	// here i will use the provided formula from the word doc

	private int getNumTuples() {
		// some code goes here
		// compute the number of tuples based on page size and tuple size
		int numberOfTuples = (int) Math.floor((BufferPool.getPageSize() * 8) / (td.getSize() * 8 + 1));
	...
	public Iterator<Tuple> iterator() {
		...
		ArrayList<Tuple> ArrayOfTuples = new ArrayList<Tuple>();
		for (int i = 0; i < numSlots; i++) {
			if (isSlotUsed(i))
				ArrayOfTuples.add(tuples[i]);
		}
		Iterator<Tuple> TupleIterator = ArrayOfTuples.iterator();
